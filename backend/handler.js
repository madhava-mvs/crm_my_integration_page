...
module.exports.InsertLead = async (event) => {
  ...
  let sql = "insert into crm.tblleads(txtSuffix, txtFirstName, txtLastName, txtCompanyName, txtPhone, txtEmail, txtAddress, refCreatedby, dtCreatedOn) VALUES('"+suffix+"', '" + firstname + "','" + lastname + "','" + company + "','" + phone + "','" + email + "','" + address + "','" + createdBy + "','" + createdOn + "');";
    ...
    let prom = await new Promise((resolve, reject) => {
      ...
      con.query(sqlexist, function(err, result){
        ...
        if(result == ""){
          ...
          resolve("email id already exists")
